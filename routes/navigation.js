...
router.route('/reportClosure')
    ...
    .post(async (req, res) => {
        ...
        try { on_street_name = helpers.checkString(xss(body.on_street_name), 'Current street'); }
        catch (e) { errors.push(String(e)); }
        try { from_street_name = helpers.checkString(xss(body.from_street_name), 'From street'); }
        catch (e) { errors.push(String(e)); }
        try { to_street_name = helpers.checkString(xss(body.to_street_name), 'To street'); }
        catch (e) { errors.push(String(e)); }
        ...
        if (typeof body.work_end_date === 'string' && body.work_end_date.trim().length > 0) {
            try {
                work_end_date = helpers.checkDateString(xss(body.work_end_date.trim()), 'Expected end date');
